...
function VideoSwap() {
    ...
    const { startPolling, stopPolling } = useSwapTaskPolling({
        onProgress: (pct) => {
            setProgress(pct);
            setMessage(`AI đang xử lý video... ${pct}%`);
        },
        ...
        <div className="flex flex-col lg:flex-row w-full gap-8 justify-center items-start">
            <div className="flex flex-col w-full lg:w-[350px] shrink-0 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 dark:bg-gray-700 dark:text-white">
                    ...
                    <div
                        className="border-dashed outline-dashed outline-2 outline-gray-300 border-gray-200 rounded-xl bg-[#fafafa] flex flex-col items-center justify-center py-10 cursor-pointer dark:bg-gray-500 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors"
            ...
            <div className="w-full bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col dark:bg-gray-700">
                <div className="flex-1 rounded-xl bg-gray-100 overflow-hidden relative group flex items-center justify-center">
                    {resultVideoSrc ? (
                        <video src={resultVideoSrc} controls autoPlay className="max-w-full max-h-full object-contain" />
                    ) : (
                        <>
                            <img
                                src={videoAI}
