...
function SwapTabs({ tab, setTab }) {
    return (
        <div className="flex flex-wrap gap-4 mb-8 py-10 transition-all duration-300 ease-in-out">
            {TABS.map(t => (
                <button
                    key={t.id}
                    onClick={() => setTab(t.id)}
                    className={`py-5 px-8 mb-9 rounded-full text-xl font-medium transition-colors duration-300 ease-in-out transform hover:scale-105 ${
                        tab === t.id
