...
function VideoSwap() {
    ...
    const handleSwap = async () => {
        ...
        try {
            ...
            await swapService.uploadMediaToTask(targetVideo, taskId, 'target');
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-stretch w-full">
            ...
            <div className="flex-1 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                    <UploadBox
                        label="1. Video gốc cần thay khuôn mặt"
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl p-6 flex flex-col gap-4 shadow-xl">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="text-left flex-1">
                            <h3 className="text-base font-bold text-slate-800 dark:text-white">Cấu hình Swap Video</h3>
                            <p className="text-xs text-slate-500 dark:text-slate-400">AI tự động tách frame, hoán đổi khuôn mặt và tái cấu trúc video chất lượng cao.</p>
                        </div>
                        <button
                            onClick={handleSwap}
            ...
            <div className="w-full lg:w-[480px] xl:w-[520px] shrink-0 flex flex-col gap-6">
                <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 overflow-hidden flex flex-col shadow-xl rounded-3xl min-h-[480px]">
                    <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800/60 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/20">
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-200">Kết quả Swap Video</span>
                        {resultVideoSrc && (
                    ...
                    <div className="relative flex-1 min-h-[380px] bg-slate-50 dark:bg-slate-950/30 flex items-center justify-center overflow-hidden">
                        {resultVideoSrc ? (
                            <video src={resultVideoSrc} controls autoPlay className="max-w-full max-h-[500px] object-contain animate-fade-in" />
                        ) : (
                            <>
                                <img src={videoAI} alt="Video mẫu" className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${isLoading ? 'opacity-30' : 'opacity-60'}`} />
                                {isLoading
                                    ? <SwapProcessingOverlay progress={progress} label="AI đang xử lý video..." />
                                    : <span className="relative z-10 text-xs text-white bg-slate-900/60 px-4 py-2.5 rounded-full backdrop-blur-md font-medium border border-white/10">Kết quả video sẽ hiện ở đây sau khi swap</span>
                                }
                            </>
                        )}
                    </div>
